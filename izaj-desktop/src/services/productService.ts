...
export class ProductService {
 ...
 static async fetchMediaUrl(session: Session | null, productId: string): Promise<string[]> {
  ...
  static async updateProductDescription(session: Session | null, productId: string, description: string): Promise<void> {
    if (!session?.access_token) {
      throw new Error('No session available');
    }

    // Use the general product update endpoint which supports description updates
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to update description: ${errorText}`);
    }
  }

  static async updateProductMedia(session: Session | null, productId: string, files: File[]): Promise<{ success: boolean; message?: string }> {
    if (!session?.access_token) {
      throw new Error('No session available');
    }
  ...
  static async updateProductMediaUrls(session: Session | null, productId: string, mediaUrls: string[]): Promise<void> {
    if (!session?.access_token) {
      throw new Error('No session available');
    }

    // Use the general product update endpoint to update media_urls
    const response = await fetch(`${API_URL}/api/products/${productId}`, {
      method: 'PUT',
      headers: this.getHeaders(session),
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to update media URLs: ${errorText}`);
    }
  }
